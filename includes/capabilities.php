...
function econozel_map_article_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'edit_econozel_articles' :
			...
			// Econozel admins can edit one's own Articles
		...
		case 'edit_econozel_article' :
			...
			if ( ! empty( $article ) ) {
				...
				if ( econozel_is_user_article_author( $user_id, $article ) && user_can( $user_id, $post_type->cap->edit_posts ) ) {
					$caps[] = 'read';
		...
		case 'econozel_articles_admin' :
			...
			// Econozel admins can enter the admin
			if ( econozel_check_admin_access( $user_id ) ) {
				$caps = array( 'read' );
			} else {
				$caps = array( 'econozel_editor' );
...
function econozel_map_post_tag_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'assign_post_tags' :
			...
			// Econozel admins can assign tags
			if ( econozel_check_admin_access( $user_id ) ) {
				$caps = array( 'read' );
