 ...
 */
function econozel_is_install() {
	return ! econozel_get_db_version_raw();
}
 ...
 * @return bool True if update, False if not
 */
function econozel_is_update() {
	$raw    = (int) econozel_get_db_version_raw();
	$cur    = (int) econozel_get_db_version();
	$retval = (bool) ( $raw < $cur );
	return $retval;
}
 ...
 */
function econozel_is_activation( $basename = '' ) {
	global $pagenow;
 ...
 * @since 1.0.0
 */
function econozel_setup_updater() {

	// Bail if no update needed
	if ( ! econozel_is_update() )
		return;

	// Call the automated updater
	econozel_version_updater();
}
 ...
 */
function econozel_version_updater() {
